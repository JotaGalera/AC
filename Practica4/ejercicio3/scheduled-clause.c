...
int main(int argc, char ** argv)
{
  ...
  #pragma omp parallel
  {
    #pragma omp single
    {
      ...
      int din_var = omp_get_dynamic();//dyn-var
      int num_threads = omp_get_max_threads();//nthreads-var
      int thread_limit_var = omp_get_thread_limit();//thread-limit-var
      printf("DENTRO de la region parallel:\n dyn-var: %d\n nthreads-var: %d\nthread-limit-var: %d\n",din_var,num_threads,thread_limit_var);
      omp_sched_t kind;
      int modifier;
  ...
  int num_threads = omp_get_max_threads();//nthreads-var
  int thread_limit_var = omp_get_thread_limit();//thread-limit-var
  printf("FUER de la region parallel:\n dyn-var: %d\n nthreads-var: %d\nthread-limit-var: %d\n",din_var,num_threads,thread_limit_var);
  omp_sched_t kind;
  int modifier;
  omp_get_schedule(&kind,&modifier);
  printf("\n run-sched-var: ");
